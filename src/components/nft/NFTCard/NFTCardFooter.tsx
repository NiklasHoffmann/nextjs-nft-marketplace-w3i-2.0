      ...
      {categories.length > 0 && (
        <div className="flex flex-wrap gap-1 min-w-0">
          {categories.slice(0, 1).map((cat, index) => (
            <div 
              key={index} 
              className={`backdrop-blur-sm px-2 py-1 rounded-md shadow-md border h-6 flex items-center ring-1 ${
                enableInsights && nft?.insight?.category
                  ? 'bg-purple-100/95 border-purple-200/60 ring-purple-300/20'
                  : 'bg-white/95 border-gray-200/60 ring-gray-300/20'
              }`}
            >
              <span className={`text-xs font-medium truncate ${
                enableInsights && nft?.insight?.category ? 'text-purple-700' : 'text-gray-700'
              }`}>
                {cat}
              </span>
            </div>
          ...
          {categories.length > 1 && (
            ...
            >
              <span className={`text-xs font-medium ${
                enableInsights && nft?.insight?.category ? 'text-purple-600' : 'text-gray-500'
              }`}>
                +{categories.length - 1}
              </span>
            </div>
