...
export default function ManualRefreshControls({
  ...
  const handleEmergencyReset = () => {
    if (confirm('âš ï¸ This will clear all cached data. Are you sure?')) {
  ...
  return (
    <div className={`flex flex-col gap-3 p-4 bg-gray-50 rounded-lg ${className}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        ...
        >
          {isRefreshing === 'nft' ? 'âŸ³' : 'ðŸŽ¨'} NFT Data
        </button>
        ...
        >
          {isRefreshing === 'contract' ? 'âŸ³' : 'ðŸ“'} Contract
        </button>
        ...
        >
          {isRefreshing === 'market' ? 'âŸ³' : 'ðŸ“Š'} Market
        </button>
        ...
        >
          {isRefreshing === 'insights' ? 'âŸ³' : 'ðŸ’¡'} Insights
        </button>
      </div>
      ...
      <div className="flex gap-2">
        ...
        >
          {isRefreshing === 'all' ? 'âŸ³ Refreshing...' : 'ðŸ”„ Refresh All'}
        </button>
