/**
 * NFT Display Formatting Utilities
 * 
 * Pure functions for formatting NFT data for display.
 * No side effects, no external dependencies.
 */

import type { NftMeta } from '@/types/core/core-nft-modern';
import type { RoyaltyInfo } from '@/types/features/nft-detail';
...
export const formatCollectionDisplayName = (
    ...
    if (contractAddress) return truncateAddress(contractAddress);
 ...
 */
export const groupAttributesByType = (attributes?: NftMeta['attributes'] | null): Record<string, NonNullable<NftMeta['attributes']>[0][]> => {
    ...
    return attributes.reduce((groups, attr) => {
        ...
        return groups;
    }, {} as Record<string, NonNullable<NftMeta['attributes']>[0][]>);
};
 ...
 */
export const createShareableNFTUrl = (contractAddress: string, tokenId: string): string => {
