...
export default function NFTInsightsPanel({ contractAddress, tokenId }: NFTInsightsPanelProps) {
    ...
    // Fetch insights directly from API
    useEffect(() => {
        let mounted = true;
        ...
        async function fetchInsights() {
            try {
                setLoading(true);
                const response = await fetch(`/api/nft/insights?contractAddress=${contractAddress}&tokenId=${tokenId}`);
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <button
                    onClick={() => {
                        setLoading(true);
                        setError(null);
                        fetch(`/api/nft/insights?contractAddress=${contractAddress}&tokenId=${tokenId}`)
                            .then(res => res.json())
                            .then(data => {
                                setInsights(data.success ? data.data : null);
                                setLoading(false);
