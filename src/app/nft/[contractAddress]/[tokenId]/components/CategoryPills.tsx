...
function CategoryPills({
    ...
    const allCategories = useMemo(() => {
        ...
        if (!insightsLoading && insights && insights.category) {
        ...
        if (insightsLoading) {
            return [
                <span key="insights-loading" className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-gray-100 text-gray-600">
                    <LoadingState size="xs" variant="inline" message="Loading Insights..." />
                </span>
            ];
        }
    ...
    const insightsEditButton = useMemo(() => {
        ...
        // Check if insights exist but are empty
        const hasEmptyInsights = insights && (
            !insights.category &&
            (!insights.tags || insights.tags.length === 0) &&
            !insights.rarity &&
            !('customTitle' in insights && insights.customTitle)
        );

        if (insights && canEdit) {
            // Edit existing insights (or fill empty insights)
            return (
                <Link
                ...
                >
                    ...
                    {hasEmptyInsights ? 'Fill Insights' : 'Edit Insights'}
                </Link>
            );
