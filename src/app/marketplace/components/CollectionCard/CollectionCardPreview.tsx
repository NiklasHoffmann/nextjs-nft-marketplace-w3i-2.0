    ...
    if (!previewImages || previewImages.length === 0) {
        return (
            <div className={rootClassName}>
                <div className="flex items-center justify-center h-full text-gray-500 bg-white/70 backdrop-blur-sm border border-black/10 rounded-md">
        ...
        <div className={rootClassName}>
            <div className="relative z-10 grid grid-cols-2 gap-1 h-full p-0">
                {previewImages.slice(0, 4).map((imageUrl: string, imgIndex: number) => (
                    <div key={imgIndex} className="rounded-md overflow-hidden bg-white/90 backdrop-blur-sm shadow-sm border border-black/10">
                        <OptimizedNFTImage
                            imageUrl={imageUrl}
